...
import { MdOutlineGridView, MdOutlineFeed, MdOutlineReviews, MdOutlineCottage, MdBookmarkBorder, MdInfoOutline } from "react-icons/md";
    ...
    <>
      <Stack as='aside'  position='relative' bg='gray.500'>
        <Stack p='1rem' position='sticky' top='0px' >
          ...
          <UnorderedList marginX='0'
            css={animation ? buttonCss : 'width: 200px'} marginTop='3rem !important' listStyleType='none' display='flex' flexDirection='column' gap='1rem'>
            {
              navItems.map((navItem, index) => (
                <Link to={navItem.link} key={index}>
                  ...
                  >
                    ...
                    <Text visibility={isOpen ? 'visible' : 'hidden'} >{navItem.name}</Text>
                  </ListItem>
                </Link>
              ))
            }
